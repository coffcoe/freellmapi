...
>
> ✅ **本任务已闭环**（commit `e59ea8f`，`MERGE-REFACTOR-PLAN.md` §3.1~§3.11 齐全）。

---

> ## ⚠️ 2026-08-08 权限修订（领航员裁定，效力高于下文 §9.2 / §11 原文）
>
> **原限制「NPC 不能直接 push 代码 / 只出方案」已解除，改为分级放开。**
>
> **修订理由**：原限制的立法意图是「NPC 尚不熟悉本仓定制，怕误改」。经 merge 改造方案一轮协作，NPC 已充分了解本仓 9 项定制与上游差异，前提消失。
>
> **新规则**：
> 1. NPC **可以直接改代码**，但仅限当前批次已解锁的任务（见 `NPC-TASK-tech-debt-dispatch.md` §1 批次表）。
> 2. 交付走 **`npc/<任务号>` 独立分支**，**不直接 push `merge-upstream`**；由灰狐跑全量回归验证后合并。
> 3. **`§9.1「整体验证不外包」原则不变`** —— NPC 自测是必要条件，不是充分条件。
> 4. 未解锁批次、以及 `anthropic.ts` / `proxy.ts` / `lib/scene.ts` / `CUSTOM-PATCHES.md` 仍**只出方案不改代码**。
>
> **下文 §9.2 与 §11 中「不能直接 push 代码」条目已作废**，保留原文仅为记录立法沿革。
...
2. 不能删除任何我们的定制函数
3. ~~不能直接 push 代码~~ → **2026-08-08 作废**，见文首权限修订：改代码已放开（分级），但须走 `npc/*` 分支、不直推 `merge-upstream`
...
- ~~**不要直接 push 代码**，只出方案文档（`MERGE-REFACTOR-PLAN.md`）。~~
  → **2026-08-08 修订**：改代码已分级放开。已解锁批次可直接改，走 `npc/<任务号>` 分支交付，**不直推 `merge-upstream`**；未解锁批次仍只出方案。详见文首权限修订与 `NPC-TASK-tech-debt-dispatch.md` §1 批次表。
- **不要删除**任何我们的定制函数，即使你认为"上游已有等价实现"。
- **不要动** `CUSTOM-PATCHES.md`。
- 如果某个定制与上游新逻辑**确实冲突**，明确写出冲突点和**两种兼容方案**，让我们决策。
- 如果某个上游新功能（如 in-flight leases）与我们现有逻辑**不兼容**，给出 fallback 方案。
