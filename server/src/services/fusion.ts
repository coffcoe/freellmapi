 ...
 */
export function selectPanel(config: FusionConfig, requirements: { requireTools?: boolean; requireVision?: boolean } = {}): { panel: FusionCandidate[]; overflow: FusionCandidate[]; dropped: string[] } {
  const maxK = panelMaxK();
...
export async function runFusion(params: {
  ...
  const { panel, overflow, dropped } = selectPanel(config, { requireTools, requireVision: vision });
  if (panel.length === 0) {
    const hint = vision
